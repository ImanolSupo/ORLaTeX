...
def main():
    ...
    guide = compile_source(ROOT / "docs/orlatex.tex", "pdflatex", output)
    preview = compile_source(ROOT / "docs/preview.tex", "pdflatex", output)
    shutil.copyfile(guide, ROOT / "docs/orlatex.pdf")
    images = ROOT / "docs/images"
    images.mkdir(parents=True, exist_ok=True)
    subprocess.run([renderer, "-scale-to", "1400", "-singlefile", "-png", str(preview), str(images / "quickstart")], check=True, timeout=60)
    inputs = ["orlatex.sty", "orlatex-input.code.tex", "docs/orlatex.tex", "docs/preview.tex", "examples/fixtures/planning-model.tex", "examples/fixtures/responsive-model.tex", "examples/fixtures/tag-target.tex", "examples/fixtures/summation-model.tex", "scripts/prepare_assets.py"]

...
\hypersetup{pdftitle={ORLaTeX user guide: experimental version 0.1.0},
  pdfauthor={Imanol Felix Supo Mamani},pdfsubject={Optimization model typesetting}}
...
\title{ORLaTeX: write the model, not the layout\\[4pt]
  {\large Also known as ORTeX and ORmath.}}
\author{Imanol Felix Supo Mamani}
\date{Experimental version 0.1.0, September 16, 2026}
\makeatletter
...
\begin{document}
\maketitle
\section{What is ORLaTeX?}
ORLaTeX is a lightweight, width-aware toolkit for mathematical optimization
...
Recommended loading: \verb|\usepackage{orlatex}|. Optional loaders: \key{ortex},
\key{ormath}; each shares the canonical implementation.

\section{Quick start}
Copy \key{orlatex.sty} and \key{orlatex-input.code.tex} beside your document.
...
ORLaTeX does not automatically split arbitrary mathematical expressions.
...
User punctuation remains literal; no period is inserted before the domain.
...
\begin{sourceexample}
\begin{verbatim}
\orlatexstyle{paper}{density=compact,row-gap=1.5pt}
...
\end{verbatim}
\end{sourceexample}
\verb|\orlatexsetup{density=normal}| changes document defaults. Setup and style
...
\key{density} & relaxed, normal, compact, dense & compact & Set row, outer, and domain spacing together.\\
...
\key{domain-gap} & dimension & 8pt & Model fit gap and notation prose-domain gap.\\
...
and \key{domain-position}. A row style may contain only those four keys.
...
ORLaTeX environments; prior meanings are restored afterwards. Namespaced
commands diagnose use outside the required context.

\subsection{Package loading}
Recommended loading remains \verb|\usepackage{orlatex}|. The compatibility
loaders are listed below; each loads the same implementation.
\par\noindent\begin{tabular}{@{}ll@{}}
Recommended & \verb|\usepackage{orlatex}|\\
Compatibility & \verb|\usepackage{ortex}|\\
Compatibility & \verb|\usepackage{ormath}|
\end{tabular}\par If using an alias, copy its wrapper file alongside the
two canonical runtime files. Independent and duplicate loading are tested.
No claim is made about CTAN availability or uniqueness of the names.
...
Version 0.1.0 is experimental. ORLaTeX does not parse arbitrary mathematics,
infer set membership, solve models, or validate their mathematical meaning.
Explicit breaks are needed for some long expressions. Nested ORLaTeX
